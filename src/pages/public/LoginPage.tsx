...
export const LoginPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/invalid-credential' || err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password') {
        ...
        setError('Unable to sign in. Please verify your internet connection.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code !== 'auth/popup-closed-by-user') {
        setError('Google sign-in was interrupted. Please try again or use email.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <SEOHead
        title="Customer Login - PeakShield Roofing"
        description="Sign in to your PeakShield Roofing customer portal to view quote requests, inspection reports, and project updates."
      />
        ...
        <div className="w-full max-w-md bg-white rounded-3xl p-8 sm:p-10 border border-stone-200/90 shadow-xl">
          <div className="text-center mb-8">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Access your quote history, inspection status, and project details.
            </p>
          </div>
